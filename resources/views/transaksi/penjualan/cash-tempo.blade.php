...
<div class="ibox animated fadeInDown">
    <div class="ibox-head">
        <div class="ibox-title"><i class="fa fa-clone mr-2"></i>Detail Penjualan Cash Tempo</div>
    </div>
    <div class="ibox-body">
        <div class="table-responsive">
	        ...
	        <div>
	            ...
	            <a type="button" class="btn btn-primary text-white" data-toggle="modal" data-target="#modal-detail-kredit" style="z-index: 10;"><i class="fa fa-plus-square-o mr-2"></i>Uang Muka</a>
	            <a type="button" class="btn btn-warning text-white" data-toggle="modal" data-target="#modal-rencana-bayar" style="z-index: 10;"><i class="fa fa-plus-square-o mr-2"></i>Rencana Pembayaran</a>
	        </div>
	        @endif
			...
			<div class="col-md-12 text-right pr-0">
		    	...
		    	@else
		    	<a href="{{url_admin('penjualan/cash-tempo')}}/{{$p->uuid}}/spjb" class="btn btn-default text-white"><i class="fa fa-download mr-1"></i>SPJB</a>
		    	<a href="{{url_admin('penjualan/cash-tempo')}}/{{$p->uuid}}/kwitansi" class="btn btn-default text-white" target="__blank"><i class="fa fa-file-text-o mr-1"></i>Kwitansi DP</a>
		    	@endif
		    </div>
		</div>
    </div>
...
@section('modal')
@if(ucc() && $p->status == 1)
{{Html::bsModalOpenLg('penjualan/cash-tempo/'.$p->uuid.'/add', 'tambah-data', 'Tambah Data Kavling')}}
	{{Form::bsRoText('project', 'Project', $p->project, '')}}
	{{Form::bsSelect2('no', 'No Kavling',$pi,'true','')}}
	{{Form::bsText('disc', 'Potongan (%)', 0, '','angka')}}
{{Html::bsModalClose('Simpan')}}

{{Html::bsModalOpenLg('penjualan/cash-tempo/'.$p->uuid.'/update', 'detail-kredit', 'Detail Uang Muka')}}
	{{Form::bsRoText('total_harga', 'Total Harga', '', '','rupiah')}}
	{{Form::bsText('uang_muka', 'Uang Muka', '', 'true','rupiah')}}
...
{{Html::bsModalOpenLg('penjualan/cash-tempo/'.$p->uuid.'/rencana-bayar', 'rencana-bayar', 'Rencana Pembayaran')}}
	{{Form::bsTextArea('ket', 'Keterangan', '', 'true','')}}
	{{Form::bsText('jml_bayar', 'Nilai Bayar', '', 'true','rupiah')}}
	{{Form::bsText('tgl_bayarR', 'Tanggal Bayar', date('d-m-Y'), 'true','tanggal datepicker')}}
{{Html::bsModalClose('Simpan')}}

{{Html::bsModalOpen('penjualan/cash-tempo/'.$p->uuid.'/posting', 'posting', 'Simpan Data')}}
...
@if(ucd() && $p->status == 1)
{{Html::bsModalOpenLg('penjualan/cash-tempo/'.$p->uuid.'/delete', 'delete-data', 'Hapus Data')}}
	{{Form::bsHidden('uuidD')}}
	{{Form::bsRoText('projectD', 'Project', $p->project, '')}}
	{{Form::bsRoText('noD', 'No Kavling','','','')}}
{{Html::bsModalClose('Hapus')}}

{{Html::bsModalOpenLg('penjualan/cash-tempo/'.$p->uuid.'/hapus', 'hapus-data', 'Hapus Data')}}
	{{Form::bsHidden('uuidH')}}
	{{Form::bsRoTextArea('ketH', 'Keterangan', '', '','')}}
	{{Form::bsRoText('jml_bayarH', 'Nilai Bayar', '', '','')}}
	{{Form::bsRoText('tgl_bayarH', 'Tanggal Bayar', '', '','')}}
{{Html::bsModalClose('Hapus')}}
@endif()
@endsection()
...
<script type="text/javascript">
	$(document).ready(function(){
        ...
        $("#table_kredit").DataTable({
			...
			ajax: "{{url_admin('penjualan/cash-tempo')}}/{{$p->uuid}}/datatable",
			columns: [
				{ data: 'data'}
			]
		});
		...
		$('#tambah-data').ajaxForm({
		    ...
		    success:function(data){
		        ...
		        $('#table_kredit').DataTable().ajax.url("{{url_admin('penjualan/cash-tempo')}}/{{$p->uuid}}/datatable").load();
		        if (data.success){
		            {{Html::jsAlertTrue()}}
		        }else{
		            {{Html::jsAlertFalse()}}
		...
		{{Html::jsShowModal('delete-data')}}
			...
			$.ajax({
                url:"{{url_admin('penjualan/cash-tempo')}}/{{$p->uuid}}/get-record/"+uuid,
                success:function(data){
		...
		{{Html::jsShowModal('detail-kredit')}}
			$.ajax({
                url:"{{url_admin('penjualan/cash-tempo')}}/{{$p->uuid}}/get-detail",
                success:function(data){
					{{Html::jsValueForm('total_harga','money','data.total')}}
					{{Html::jsValueForm('uang_muka','money','data.pjl.uang_muka')}}
		...
		{{Html::jsShowModal('rencana-bayar')}}
			$.ajax({
                url:"{{url_admin('penjualan/cash-tempo')}}/{{$p->uuid}}/get-detail",
                success:function(data){
					{{Html::jsValueForm('jml_bayar','money','data.sb')}}
				},
                error:function(data){
                    $("#modal-rencana-bayar").modal('hide');
                    {{Html::jsAlertFail()}}
                }
            });
		{{Html::jsCloseModal()}}
		$('#rencana-bayar').ajaxForm({
		    beforeSubmit:function(){
		        $(".submit").attr("disabled", true);
		    },
		    success:function(data){
		...
		{{Html::jsShowModal('hapus-data')}}
			var rb = $(e.relatedTarget).data('uuid');
			$.ajax({
                url:"{{url_admin('penjualan/cash-tempo')}}/{{$p->uuid}}/get-rencana-bayar/"+rb,
                success:function(data){
					{{Html::jsValueForm('uuidH','input','rb')}}
					{{Html::jsValueForm('ketH','input','data.ket')}}
					{{Html::jsValueForm('jml_bayarH','money','data.rencana_bayar')}}
					{{Html::jsValueForm('tgl_bayarH','date','data.rencana_tgl_bayar')}}
				},
                error:function(data){
