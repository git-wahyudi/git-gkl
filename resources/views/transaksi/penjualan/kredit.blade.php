...
<div class="ibox animated fadeInDown">
    <div class="ibox-head">
        <div class="ibox-title"><i class="fa fa-clone mr-2"></i>Detail Penjualan Kredit</div>
    </div>
    <div class="ibox-body">
        <div class="table-responsive">
	        ...
	        <div>
	            ...
	            <a type="button" class="btn btn-primary text-white" data-toggle="modal" data-target="#modal-detail-kredit" style="z-index: 10;"><i class="fa fa-plus-square-o mr-2"></i>Input Detail Kredit</a>
	        </div>
	        @endif
			...
			<div class="col-md-12 text-right pr-0">
		    	...
		    	@else
		    	<a href="{{url_admin('penjualan/kredit')}}/{{$p->uuid}}/spjb" class="btn btn-default text-white"><i class="fa fa-download mr-1"></i>SPJB</a>
		    	<a href="{{url_admin('penjualan/kredit')}}/{{$p->uuid}}/kwitansi" class="btn btn-default text-white" target="__blank"><i class="fa fa-file-text-o mr-1"></i>Kwitansi DP</a>
		    	@endif
		    </div>
		</div>
    </div>
...
@section('modal')
@if(ucc() && $p->status == 1)
{{Html::bsModalOpenLg('penjualan/kredit/'.$p->uuid.'/add', 'tambah-data', 'Tambah Data Kavling')}}
	{{Form::bsRoText('project', 'Project', $p->project, '')}}
	{{Form::bsSelect2('no', 'No Kavling',$pi,'true','')}}
{{Html::bsModalClose('Simpan')}}

{{Html::bsModalOpenLg('penjualan/kredit/'.$p->uuid.'/update', 'detail-kredit', 'Detail Kredit')}}
	{{Form::bsRoText('total_harga', 'Total Harga', '', 'rupiah','')}}
	{{Form::bsText('uang_muka', 'Uang Muka', '', 'true','rupiah hitung')}}
	{{Form::bsSelect('cara_bayar', 'Cara Bayar (Uang Muka)', ['Tunai'=>'Tunai', 'Transfer'=>'Transfer'], 'true','')}}
	{{Form::bsRoText('sisa_piutang', 'Sisa Piutang', '', '','')}}
	{{Form::bsText('tenor', 'Tenor (Bulan)', '', 'true','rupiah hitung')}}
	{{Form::bsText('angsuran', 'Angsuran Perbulan', '', 'true','rupiah')}}
	{{Form::bsText('tgl_bayar', 'Tanggal Jatuh Tempo', '', 'true','tanggal datepicker')}}
{{Html::bsModalClose('Simpan')}}

{{Html::bsModalOpen('penjualan/kredit/'.$p->uuid.'/posting', 'posting', 'Simpan Data')}}
...
@if(ucd() && $p->status == 1)
{{Html::bsModalOpenLg('penjualan/kredit/'.$p->uuid.'/delete', 'delete-data', 'Hapus Data')}}
	{{Form::bsHidden('uuidD')}}
...
<script type="text/javascript">
	$(document).ready(function(){
        ...
        $("#table_kredit").DataTable({
			...
			ajax: "{{url_admin('penjualan/kredit')}}/{{$p->uuid}}/datatable",
			columns: [
				{ data: 'data'}
			]
		});
		...
		$('#tambah-data').ajaxForm({
		    ...
		    success:function(data){
		        ...
		        $('#table_kredit').DataTable().ajax.url("{{url_admin('penjualan/kredit')}}/{{$p->uuid}}/datatable").load();
		        if (data.success){
		            {{Html::jsAlertTrue()}}
		        }else{
		            {{Html::jsAlertFalse()}}
		...
		{{Html::jsShowModal('delete-data')}}
			...
			$.ajax({
                url:"{{url_admin('penjualan/kredit')}}/{{$p->uuid}}/get-record/"+uuid,
                success:function(data){
		...
		{{Html::jsShowModal('detail-kredit')}}
			$.ajax({
                url:"{{url_admin('penjualan/kredit')}}/{{$p->uuid}}/get-detail",
                success:function(data){
					{{Html::jsValueForm('total_harga','money','data.total')}}
					{{Html::jsValueForm('uang_muka','money','data.pjl.uang_muka')}}
					{{Html::jsValueForm('cara_bayar','select','data.pjl.cara_bayar')}}
					{{Html::jsValueForm('sisa_piutang','money','data.pjl.sisa_piutang')}}
					{{Html::jsValueForm('tenor','money','data.pjl.tenor')}}
					{{Html::jsValueForm('angsuran','money','data.pjl.angsuran')}}
					{{Html::jsValueForm('tgl_bayar','date','data.pjl.tgl_bayar')}}
				},
                error:function(data){
